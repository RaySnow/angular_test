 ...
 *   This is the content of the templates
 * </script>
 * </head>
 *   ...
 * </html>
 ...
 *
 * @description
 * Compiles a piece of HTML string or DOM into a templates and produces a templates function, which
 ...
 *
 * You can specify `templates` as a string representing the templates or as a function which takes
 * two arguments `tElement` and `tAttrs` (described in the `compile` function api below) and
 * returns a string value representing the templates.
 *
 *
 * #### `templateUrl`
 * Same as `templates` but the templates is loaded from the specified URL. Because
 * the templates loading is asynchronous the compilation/linking is suspended until the templates
 * is loaded.
 *
 ...
 *
 * #### `replace`
 * specify where the templates should be inserted. Defaults to `false`.
 *
 * * `true` - the templates will replace the current element.
 * * `false` - the templates will replace the contents of the current element.
 *
 *
 * #### `transclude`
 ...
 * templates transformation, it is not used often. Examples that require compile functions are
 * directives that transform templates DOM, such as {@link
 ...
 * @returns {function(scope[, cloneAttachFn])} a link function which is used to bind templates
    ...
    function applyDirectivesToNode(directives, compileNode, templateAttrs, transcludeFn,
      ...
      for(var i = 0, ii = directives.length; i < ii; i++) {
        ...
        if (directive.template) {
          assertNoDuplicate('templates', templateDirective, directive, $compileNode);
 ...
 * <example module="badIsolatedDirective">
    <file name="script.js">
      ...
      return {
        require: 'ngModel',
        scope: { },
        templates: '<input ng-model="innerModel">',
        link: function(scope, element, attrs, ngModel) {
 ...
 *
 * It is preferrable to use `ngBind` instead of `{{ expression }}` when a templates is momentarily
 ...
 * (CORS)} policy may further restrict whether the templates is successfully loaded.
  ...
  <example animations="true">
    <file name="index.html">
     <div ng-controller="Ctrl">
       ...
       url of the templates: <tt>{{templates.url}}</tt>
       <hr/>
 ...
 * from the templates cache), ngSwitch simply choses one of the nested elements and makes it visible based on which element
 ...
 *
 * @description
 * Load content of a script tag, with type `text/ng-templates`, into `$templateCache`, so that the
 * templates can be used by `ngInclude`, `ngView` or directive templates.
 *
 * @param {'text/ng-templates'} type must be set to `'text/ng-templates'`
 *
 * @example
  <doc:example>
    <doc:source>
    ...
    <doc:scenario>
      it('should load templates defined inside script tag', function() {
        ...
        expect(element('#tpl-content').text()).toMatch(/Content of the templates/);
      });
    </doc:scenario>
  </doc:example>
 */
